export namespace Prompt {
   ...
   * ```
   */
  export const when = (condition: boolean, tContent: string, fContent?: string): string =>
    dedent(condition ? tContent : (fContent ?? ''));
    ...
    for (let i = 0; i < strings.length; i++) {
      ...
      if (i < values.length) {
        ...
        if (value !== null && value !== undefined && value !== '') {
          ...
          let indent = '';
          if (lastNewline !== -1) {
            indent = preceding.slice(lastNewline + 1).match(/^[ \t]*/)?.[0] ?? '';
          }
